...
const HeartRateScreen = ({ initialSelectedTab = 'Día', initialSelectedDate = new Date(), initialIsDatePickerVisible = false }) => {
  ...
  const getData = (tab) => {
    ...
    switch (tab) {
      case 'Mes':
        ...
        data.datasets[0].data = [70, 72, 68, 75]; // Datos de ejemplo para el mes
        summaryData.average = (70 + 72 + 68 + 75) / 4; // Calculo del promedio
        summaryData.highest = Math.max(70, 72, 68, 75); // Calculo de la más alta
        summaryData.lowest = Math.min(70, 72, 68, 75); // Calculo de la más baja
        break;
      case 'Semana':
        data.labels = ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'];
        data.datasets[0].data = [72, 70, 74, 69, 71, 73, 72]; // Datos de ejemplo para la semana
        summaryData.average = (72 + 70 + 74 + 69 + 71 + 73 + 72) / 7; // Calculo del promedio
        summaryData.highest = Math.max(72, 70, 74, 69, 71, 73, 72); // Calculo de la más alta
        summaryData.lowest = Math.min(72, 70, 74, 69, 71, 73, 72); // Calculo de la más baja
        break;
      case 'Día':
        data.labels = ['12AM', '3AM', '6AM', '9AM', '12PM', '3PM', '6PM', '9PM'];
        data.datasets[0].data = [70, 72, 68, 75, 73, 70, 69, 74]; // Datos de ejemplo para el día
  ...
  return (
    ...
    >
      ...
      <TouchableOpacity style={styles.button}>
        <Text style={styles.buttonText}>Medición del Ritmo Cardíaco</Text>
      </TouchableOpacity>
      <TouchableOpacity style={styles.menuItem} onPress={() => router.navigate('HeartRateAnalysis')}>
        <Icon name="analytics-outline" size={25} color="#000" />
        <Text style={styles.menuText}>Análisis del Ritmo Cardíaco</Text>
