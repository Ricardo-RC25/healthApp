...
const HeartRateScreen = ({ initialSelectedTab = 'Día' }) => {
  ...
  const getData = (tab) => {
    ...
    switch (tab) {
      case 'Mes':
        ...
        data.datasets[0].data = [70, 72, 68, 75];
        summaryData.average = (70 + 72 + 68 + 75) / 4;
        summaryData.highest = Math.max(70, 72, 68, 75);
        summaryData.lowest = Math.min(70, 72, 68, 75);
        break;
      case 'Semana':
        data.labels = ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'];
        data.datasets[0].data = [72, 70, 74, 69, 71, 73, 72];
        summaryData.average = (72 + 70 + 74 + 69 + 71 + 73 + 72) / 7;
        summaryData.highest = Math.max(72, 70, 74, 69, 71, 73, 72);
        summaryData.lowest = Math.min(72, 70, 74, 69, 71, 73, 72);
        break;
      case 'Día':
        data.labels = ['12AM', '3AM', '6AM', '9AM', '12PM', '3PM', '6PM', '9PM'];
        data.datasets[0].data = [70, 72, 68, 75, 73, 70, 69, 74];
  ...
  return (
    ...
    >
      <View style={[styles.heartRateContainer, { backgroundColor: isDarkMode ? '#1e1e1e' : '#ffffff' }]}>
      ...
      <TouchableOpacity style={[styles.menuItem, { backgroundColor: isDarkMode ? '#1e1e1e' : '#ffffff' }]} onPress={() => router.navigate('HeartRateAnalysis')}>
        <Icon name="analytics-outline" size={25} color={isDarkMode ? '#ffffff' : '#000000'} />
        <Text style={[styles.menuText, { color: isDarkMode ? '#ffffff' : '#000000' }]}>Análisis del Ritmo Cardíaco</Text>
