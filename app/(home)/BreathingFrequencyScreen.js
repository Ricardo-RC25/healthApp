...
const RespiratoryRateScreen = ({ initialSelectedTab = 'Día', initialSelectedDate = new Date(), initialIsDatePickerVisible = false }) => {
  ...
  const getData = (tab) => {
    ...
    switch (tab) {
      case 'Mes':
        ...
        data.datasets[0].data = [14, 16, 15, 13]; // Datos de ejemplo para el mes
        summaryData.average = (14 + 16 + 15 + 13) / 4; // Cálculo del promedio
        summaryData.highest = Math.max(14, 16, 15, 13); // Cálculo de la más alta
        summaryData.lowest = Math.min(14, 16, 15, 13); // Cálculo de la más baja
        break;
      case 'Semana':
        data.labels = ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'];
        data.datasets[0].data = [14, 15, 16, 15, 14, 13, 16]; // Datos de ejemplo para la semana
        summaryData.average = (14 + 15 + 16 + 15 + 14 + 13 + 16) / 7; // Cálculo del promedio
        summaryData.highest = Math.max(14, 15, 16, 15, 14, 13, 16); // Cálculo de la más alta
        summaryData.lowest = Math.min(14, 15, 16, 15, 14, 13, 16); // Cálculo de la más baja
        break;
      case 'Día':
        data.labels = ['12PM', '3PM', '6PM', '9PM'];
        data.datasets[0].data = [15, 16, 14, 13, 15, 14, 16, 15]; // Datos de ejemplo para el día
        summaryData.average = data.datasets[0].data.reduce((a, b) => a + b, 0) / data.datasets[0].data.length; // Cálculo del promedio
        summaryData.highest = Math.max(...data.datasets[0].data); // Cálculo de la más alta
        summaryData.lowest = Math.min(...data.datasets[0].data); // Cálculo de la más baja
        break;
      default:
        break;
    }
  ...
  return (
    ...
    >
      <View style={styles.respiratoryRateContainer}>
      ...
      <TouchableOpacity style={styles.button}>
        <Text style={styles.buttonText}>Medición de la Frecuencia Respiratoria</Text>
      </TouchableOpacity>
