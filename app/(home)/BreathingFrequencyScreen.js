...
const RespiratoryRateScreen = ({ initialSelectedTab = 'Día' }) => {
  ...
  const getData = (tab) => {
    ...
    switch (tab) {
      case 'Mes':
        ...
        data.datasets[0].data = [14, 16, 15, 13];
        summaryData.average = (14 + 16 + 15 + 13) / 4;
        summaryData.highest = Math.max(14, 16, 15, 13);
        summaryData.lowest = Math.min(14, 16, 15, 13);
        break;
      case 'Semana':
        data.labels = ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'];
        data.datasets[0].data = [14, 15, 16, 15, 14, 13, 16];
        summaryData.average = (14 + 15 + 16 + 15 + 14 + 13 + 16) / 7;
        summaryData.highest = Math.max(14, 15, 16, 15, 14, 13, 16);
        summaryData.lowest = Math.min(14, 15, 16, 15, 14, 13, 16);
        break;
      case 'Día':
        data.labels = ['12PM', '3PM', '6PM', '9PM'];
        data.datasets[0].data = [15, 16, 14, 13, 15, 14, 16, 15];
  ...
  return (
    ...
    >
      <View style={[styles.respiratoryRateContainer, { backgroundColor: isDarkMode ? '#1e1e1e' : '#ffffff' }]}>
      ...
      <Text style={[styles.menuText, { color: isDarkMode ? '#ffffff' : '#000000' }]}>Análisis de frecuencia respiratoria</Text>
