...
const StepsScreen = ({ initialSelectedTab = 'Día' }) => {
  ...
  const getData = (tab) => {
    ...
    switch (tab) {
      case 'Mes':
        ...
        data.datasets[0].data = [6000, 7000, 5000, 8000];
        break;
      case 'Semana':
        data.labels = ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'];
        data.datasets[0].data = [7000, 6000, 8000, 5000, 7000, 6000, 7000];
        break;
      case 'Día':
        data.labels = ['12AM', '3AM', '6AM', '9AM', '12PM', '3PM', '6PM', '9PM'];
        data.datasets[0].data = [0, 0, 1000, 2000, 3000, 4000, 3000, 2000];
        break;
      default:
        break;
    }
  ...
  return (
    ...
    >
      <View style={[styles.stepsContainer, { backgroundColor: isDarkMode ? '#1e1e1e' : '#ffffff' }]}>
      ...
      <TouchableOpacity style={[styles.menuItem, { backgroundColor: isDarkMode ? '#1e1e1e' : '#ffffff', borderBottomColor: isDarkMode ? '#444444' : '#e0e0e0' }]} onPress={() => router.navigate('StepsAnalysis')}>
        <Icon name="analytics-outline" size={25} color={isDarkMode ? '#ffffff' : '#000000'} />
        <Text style={[styles.menuText, { color: isDarkMode ? '#ffffff' : '#000000' }]}>Análisis de pasos</Text>
        <Icon name="chevron-forward-outline" size={25} color={isDarkMode ? '#ffffff' : '#000000'} style={styles.menuIcon} />
      </TouchableOpacity>
      <TouchableOpacity style={[styles.menuItem, { backgroundColor: isDarkMode ? '#1e1e1e' : '#ffffff', borderBottomColor: isDarkMode ? '#444444' : '#e0e0e0' }]} onPress={() => setIsHistoryLogExpanded(!isHistoryLogExpanded)}>
