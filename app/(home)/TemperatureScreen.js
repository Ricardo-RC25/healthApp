...
const TemperatureScreen = ({ initialSelectedTab = 'Día' }) => {
  ...
  const getData = (tab) => {
    ...
    switch (tab) {
      case 'Mes':
        ...
        data.datasets[0].data = [36.6, 36.8, 37.0, 36.5];
        summaryData.average = (36.6 + 36.8 + 37.0 + 36.5) / 4;
        summaryData.highest = Math.max(36.6, 36.8, 37.0, 36.5);
        summaryData.lowest = Math.min(36.6, 36.8, 37.0, 36.5);
        break;
      case 'Semana':
        data.labels = ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'];
        data.datasets[0].data = [36.7, 36.5, 36.8, 37.0, 36.6, 36.9, 36.4];
        summaryData.average = (36.7 + 36.5 + 36.8 + 37.0 + 36.6 + 36.9 + 36.4) / 7;
        summaryData.highest = Math.max(36.7, 36.5, 36.8, 37.0, 36.6, 36.9, 36.4);
        summaryData.lowest = Math.min(36.7, 36.5, 36.8, 37.0, 36.6, 36.9, 36.4);
        break;
      case 'Día':
        data.labels = ['12PM', '3PM', '6PM', '9PM'];
        data.datasets[0].data = [36.5, 36.7, 36.6, 36.8, 37.0, 36.9, 36.5, 36.7];
    ...
    <View style={styles.chartContainer}>
      <View style={[styles.chartArea, { borderColor: isDarkMode ? '#666666' : '#e0e0e0' }]}>
        ...
        {Array.from({ length: 5 }).map((_, i) => (
          <View key={i} style={[styles.horizontalLine, { bottom: i * 44, backgroundColor: isDarkMode ? '#444444' : '#e0e0e0' }]} />
        ))}
  ...
  return (
    ...
    >
      <View style={[styles.temperatureContainer, { backgroundColor: isDarkMode ? '#1e1e1e' : '#ffffff' }]}>
      ...
      <TouchableOpacity style={[styles.menuItem, { backgroundColor: isDarkMode ? '#1e1e1e' : '#ffffff' }]} onPress={() => router.navigate('TemperatureAnalysis')}>
        <Icon name="analytics-outline" size={25} color={isDarkMode ? '#ffffff' : '#000000'} />
        <Text style={[styles.menuText, { color: isDarkMode ? '#ffffff' : '#000000' }]}>Análisis de la Temperatura</Text>
        <Icon name="chevron-forward-outline" size={25} color={isDarkMode ? '#ffffff' : '#000000'} style={styles.menuIcon} />
      </TouchableOpacity>
      <TouchableOpacity style={[styles.menuItem, { backgroundColor: isDarkMode ? '#1e1e1e' : '#ffffff' }]} onPress={() => router.navigate('HealthSettings')}>
        <Icon name="settings-outline" size={25} color={isDarkMode ? '#ffffff' : '#000000'} />
        <Text style={[styles.menuText, { color: isDarkMode ? '#ffffff' : '#000000' }]}>Configuraciones de salud</Text>
        <Icon name="chevron-forward-outline" size={25} color={isDarkMode ? '#ffffff' : '#000000'} style={styles.menuIcon} />
      </TouchableOpacity>
    </ScrollView>
  );
