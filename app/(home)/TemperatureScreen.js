...
const TemperatureScreen = ({ initialSelectedTab = 'Día', initialSelectedDate = new Date(), initialIsDatePickerVisible = false }) => {
  ...
  const getData = (tab) => {
    ...
    switch (tab) {
      case 'Mes':
        ...
        data.datasets[0].data = [36.6, 36.8, 37.0, 36.5]; // Datos de ejemplo para el mes
        summaryData.average = (36.6 + 36.8 + 37.0 + 36.5) / 4; // Cálculo del promedio
        summaryData.highest = Math.max(36.6, 36.8, 37.0, 36.5); // Cálculo de la más alta
        summaryData.lowest = Math.min(36.6, 36.8, 37.0, 36.5); // Cálculo de la más baja
        break;
      case 'Semana':
        data.labels = ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'];
        data.datasets[0].data = [36.7, 36.5, 36.8, 37.0, 36.6, 36.9, 36.4]; // Datos de ejemplo para la semana
        summaryData.average = (36.7 + 36.5 + 36.8 + 37.0 + 36.6 + 36.9 + 36.4) / 7; // Cálculo del promedio
        summaryData.highest = Math.max(36.7, 36.5, 36.8, 37.0, 36.6, 36.9, 36.4); // Cálculo de la más alta
        summaryData.lowest = Math.min(36.7, 36.5, 36.8, 37.0, 36.6, 36.9, 36.4); // Cálculo de la más baja
        break;
      case 'Día':
        data.labels = ['12PM', '3PM', '6PM', '9PM'];
        data.datasets[0].data = [36.5, 36.7, 36.6, 36.8, 37.0, 36.9, 36.5, 36.7]; // Datos de ejemplo para el día
